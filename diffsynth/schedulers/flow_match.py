...
class FlowMatchScheduler():
    ...
    def pred_x0(self, sample, model_output, timestep):
        """model_output = (noise - sample) 일 때 x0 추정. x0 = sample - σ*model_output/(1-σ).
        training_target(noisy_latents, ...) 로 학습한 경우에만 사용. velocity = ε - x0 로 학습했으면 pred_x0_from_velocity 사용."""
        if isinstance(timestep, torch.Tensor):
            timestep = timestep.cpu()
        timestep_id = torch.argmin((self.timesteps - timestep).abs())
        sigma = self.sigmas[timestep_id]
        if isinstance(sigma, torch.Tensor) and sample.device != sigma.device:
