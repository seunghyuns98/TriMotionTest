...
class MotionTripletDataset(torch.utils.data.Dataset):
    def __init__(self, base_path, max_num_frames=81, frame_interval=1, num_frames=81, height=480, width=832, latent_path=None):
    ...
    def __getitem__(self, data_id):
        ...
        if reference_type == 'video':
            reference_latents = torch.load(video_path.replace(".mp4", "_emb.pt").replace(self.base_path, self.latent_path))['video_frame']  
        elif reference_type == 'pose':
            reference_latents = torch.load(video_path.replace(".mp4", "_emb.pt").replace(self.base_path, self.latent_path))['pose_frame']
        elif reference_type == 'text':
            if random.random() < 0.5:
                reference_latents = torch.load(video_path.replace(".mp4", "_emb.pt").replace(self.base_path, self.latent_path))['text_long_frame']
            else:
                reference_latents = torch.load(video_path.replace(".mp4", "_emb.pt").replace(self.base_path, self.latent_path))['text_short_frame']

        reference_global = torch.load(video_path.replace(".mp4", "_emb.pt").replace(self.base_path, self.latent_path))['video_global']
